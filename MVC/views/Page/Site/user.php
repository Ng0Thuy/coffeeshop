<main class="grid wide container" style="margin: 80px;">
    <div class="info-user">
        <h2 class="info-user__heading">Thông tin khách hàng</h2>
        <div class="form-info-user">
            <form action="" method="" class="change-password">
                <h2 class="change-password__heading">Đổi mật khẩu</h2>
                <div class="change-password__main">
                    <div class="change-password__item">
                        <p class="change-password__text">Mật khẩu <sup style="color:red;">*</sup></p>
                        <input type="password" name="" id="">
                        <p class="error">Mật không đúng</p>
                    </div>
                    <div class="clear"></div>
                    <div class="change-password__item">
                        <p class="change-password__text">Mật khẩu mới <sup style="color:red;">*</sup></p>
                        <input type="password" name="" id="">
                        <span class="error"></span>
                    </div>
                </div>
            </form>
            <form action="" method="" class="info-user__main">

            </form>
        </div>
    </div>
</main>